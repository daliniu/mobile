...
string ServantProxy::taf_tunneled_name() const
{
    if (_objectProxy)
    {
        return _objectProxy->getTunneledName();
    }
    return "";
}
...
void ServantProxy::taf_shmcomm_enable(bool enable)
{ 
...
{    
    ...
    if (td->data()->_hashCode != ThreadPrivateData::INVALID_HASH_CODE)
    {
        ...
        LOG->info() << "[TAF][ServantProxy::invoke, " << _objectProxy->name() << ", hash:" << td->data()->_hashCode << "]" << endl;
        cout << "[TAF][ServantProxy::invoke, " << _objectProxy->name() << ", hash:" << td->data()->_hashCode << "]" << endl;
    }

    //如果是需要染色的消息
    if (td->data()->_bDyeing)
    {
        LOG->info() << "[TAF][" << __FILE__ << "|" << __LINE__ << "] set dyeing, key=" << td->data()->_dyeingKey << endl;
    ...
    if (keyIt != req->request.status.end())
    {
        ...
        if (grid != ThreadPrivateData::INVALID_GRID_CODE)
        {
            ...
            LOG->info() << "[TAF][ServantProxy::invoke, " << _objectProxy->name() << ", grid:" << routeKey << "->" << grid << "]" << endl;
        }
    }
    ////////////////////////////////////////////////////////////////////////////////////
    ...
    if (req->type == ReqMessage::SYNC_CALL)
    {
        ...
        {
            snprintf(td->data()->_szHost, sizeof(td->data()->_szHost), "%s", req->adapter->endpoint().fulldesc().c_str());
