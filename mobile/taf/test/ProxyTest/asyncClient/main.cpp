...
class ProxyCallBack : public BServantPrxCallback
{
public:
	ProxyCallBack(){}
	...
	virtual void callback_dohandle(const std::string& ret)
	{
		TC_LockT<TC_ThreadMutex> lock(mutex);
		g_count++;
	}

	virtual void callback_dohandle_exception(taf::Int32 ret)
	{
...
int main(int argc, char *argv[])
{
	...
	while(count < iCount)
	{
		BServantPrxCallbackPtr cb = new ProxyCallBack();
		try
		{
			proxyPrx->async_dohandle(cb, "hello world");
		}
