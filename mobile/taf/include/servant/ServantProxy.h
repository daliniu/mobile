...
struct ThreadPrivateData : virtual public TC_HandleBase
{
    ...
    ThreadPrivateData() : _hashCode(INVALID_HASH_CODE), _gridCode(INVALID_GRID_CODE),_bDyeing(false) { _szHost[0] = '\0'; }

    ThreadPrivateData(const ThreadPrivateData & data)
    {
        this->_hashCode  = data._hashCode;
        this->_gridCode  = data._gridCode;
        this->_routeKey  = data._routeKey;
        this->_dyeingKey = data._dyeingKey;
        this->_sampleKey = data._sampleKey;
        this->_bDyeing   = data._bDyeing;

        snprintf(_szHost, sizeof(_szHost), "%s", data._szHost);
    }
};
...
public:
    ...
    /**
     * 获取线程私有数据
     * @return ThreadPrivateDataPtr& 
     */
    ThreadPrivateDataPtr& getPrivateData() { return _dataPtr; }
...
public:
    ...
    static string STATUS_TUNNEL_OBJ; //需要隧道传递的OBJ

    static string STATUS_HASH_CODE; //需要隧道传递的hash值
...
public:
     ...
     * @return Communicator* 
     */
    Communicator* taf_communicator() { return _comm; }
     ...
     * @return string
     */
    void taf_name(const string &name);

    /**
     * 判断是否启用隧道服务
     * @return string
     */
    bool taf_tunnel_enabled() const;
    ...
    */
     ...
     * 设置是否可用共享内存通信
     */
    void taf_bus_enable(bool enable);
