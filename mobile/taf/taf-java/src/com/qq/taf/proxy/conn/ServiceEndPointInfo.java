...
public class ServiceEndPointInfo implements Comparable<ServiceEndPointInfo>{
	...
	public void setFailed( boolean b ) {
		if ( b ) {
			failedTime = Millis100TimeProvider.INSTANCE.currentTimeMillis();
		} else {
			failedTime = 0 ;
		}
	}
	...
	public void setTimeout( boolean b ) {
		if ( b ) {
			timeoutTime = Millis100TimeProvider.INSTANCE.currentTimeMillis();
		} else {
			timeoutTime = 0 ;
		}
	}
	...
	boolean shouldTryConn() {
		if ( isFailed() ) {
			if ( Millis100TimeProvider.INSTANCE.currentTimeMillis() - failedTime > (failedWaitTimeMill)  ) {
				return true ;
			} else {
				return false ;
			}
