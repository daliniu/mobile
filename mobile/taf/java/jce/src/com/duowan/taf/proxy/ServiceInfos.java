...
import com.duowan.taf.proxy.conn.ServiceEndPoint;
...
public class ServiceInfos implements IServicesInfo {
	...
	private Map<Integer, List<ServiceEndPointInfo>> gridServices = new HashMap<Integer, List<ServiceEndPointInfo>>();

	private Map<Integer, List<ServiceEndPointInfo>> gridActiveServices = new HashMap<Integer, List<ServiceEndPointInfo>>();
	
	private List<ServiceEndPointInfo> srcActiveServices = new LinkedList<ServiceEndPointInfo>();

	private List<ServiceEndPointInfo> activeServices = new LinkedList<ServiceEndPointInfo>();
	
	private HashSet<String> activeServiceKey;

	private List<ServiceEndPointInfo> inactiveServices = new LinkedList<ServiceEndPointInfo>();

	private List<ServiceEndPointInfo> allServices = new LinkedList<ServiceEndPointInfo>();
		...
		try {
			...
			for (ServiceEndPointInfo e : allServices) {
				if (e.getKey().equals(serviceinfo.getKey())) {
					...
					activeServiceKey.remove(e.getKey());
			...
			for (ServiceEndPointInfo s : allServices) {
				if (s.getKey().equals(key)) {
					...
					if ( null == gridServices.get(newGridValue) ) {
						gridServices.put(newGridValue, new LinkedList<ServiceEndPointInfo>());
					}
					...
					if ( null == gridActiveServices.get(newGridValue) ) {
						gridActiveServices.put(newGridValue, new LinkedList<ServiceEndPointInfo>());
					}
			...
			for (ServiceEndPointInfo e : allServices) {
				if (e.getKey().equals(service.getInfo().getKey())) {
					...
					if (!activeServices.contains(e)) {
						...
						activeServiceKey.add(e.getKey());
					}
					if ( null == gridActiveServices.get(e.getGrid()) ) {
						gridActiveServices.put(e.getGrid(), new LinkedList<ServiceEndPointInfo>());
					}
	...
	/* (non-Javadoc)
	 * @see com.duowan.taf.proxy.IServicesInfo#getActiveServices()
	 */
	...
	/* (non-Javadoc)
	 * @see com.duowan.taf.proxy.IServicesInfo#getInactiveServices()
	 */
	...
	/* (non-Javadoc)
	 * @see com.duowan.taf.proxy.IServicesInfo#getGridServices(int)
	 */
	...
	/* (non-Javadoc)
	 * @see com.duowan.taf.proxy.IServicesInfo#getGridActiveServices(int)
	 */
	...
	/* (non-Javadoc)
	 * @see com.duowan.taf.proxy.IServicesInfo#getAllServices()
	 */
	...
	void setServices(ArrayList<EndpointF> activeEp, ArrayList<EndpointF> inactiveEp) {
	...
	void setServiceInfos(ArrayList<ServiceEndPointInfo> activeEp, ArrayList<ServiceEndPointInfo> inactiveEp) {
		...
		List<ServiceEndPointInfo> _activeServices = new LinkedList<ServiceEndPointInfo>();
		List<ServiceEndPointInfo> _inactiveServices = new LinkedList<ServiceEndPointInfo>();
		List<ServiceEndPointInfo> _allServices = new LinkedList<ServiceEndPointInfo>();
		List<ServiceEndPointInfo> _srcActiveServices = new LinkedList<ServiceEndPointInfo>();
		HashSet<String> _activeServiceKey = new HashSet<String>();

		for (ServiceEndPointInfo e : activeEp) {
			if (!_gridServices.containsKey(e.getGrid())) {
				_gridServices.put(e.getGrid(), new LinkedList<ServiceEndPointInfo>());
			}
			if (!_gridActiveServices.containsKey(e.getGrid())) {
				_gridActiveServices.put(e.getGrid(), new LinkedList<ServiceEndPointInfo>());
			}
			_gridServices.get(e.getGrid()).add(e);
			_gridActiveServices.get(e.getGrid()).add(e);
			_activeServices.add(e);
			_activeServiceKey.add(e.getKey());
			_allServices.add(e);
	...
	public boolean isActive(ServiceEndPointInfo info) {
		return activeServiceKey.contains(info.getKey());
	}
	
	public boolean isActive(ServiceEndPoint point) {
		return activeServiceKey.contains(point.getKey());
	}
