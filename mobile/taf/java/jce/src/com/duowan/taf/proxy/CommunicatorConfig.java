...
public class CommunicatorConfig  {
	...
	public static CommunicatorConfig getDefaultConfig() {
		...
		CommunicatorConfig config = new CommunicatorConfig(null , null , path, 3000,3000, 60000, 9 , 9);
	...
	public static CommunicatorConfig parseQueryConfig(String fileName) {
		...
		try {
			...
			if (null != temp && !temp.toLowerCase().equals("java")) {
				modulename = "java."+temp;
			} else {
				modulename = getAppFilePath();
				if ( modulename.length() == 0  ) {
					modulename = "java.java";
				} else {
		...
		try {
			CommunicatorConfig config = new CommunicatorConfig(locator , stat , modulename, syncInvokeTimeout,asyncInvokeTimeout, 
					refreshEndpointInterval, sendThreadNum , asyncthreadNum);
			config.setMaxSampleCount(maxSampleCount);
			config.setSampleRate(sampleRate);
	 ...
	 * @return
	 */
	public ProxyConfig getConfig(String content,TafThreadPool tafThreadPool) {
		String objectName = "";
		...
		ProxyConfig config = new ProxyConfig(objectName,tafThreadPool);
	...
	public static void main(String[] args) {
		String a = "jar:file:/usr/local/resin_mpet/webapps/webapp_mpet/WEB-INF/lib/taf-proxy-1.0.0.jar!/com/duowan/taf/proxy/";
