...
import com.duowan.netutil.nio.mina2.util.NamePreservingRunnable;
...
public class TafThreadPool {
	...
	private void initSendPool() {
		...
		for (int i = 0; i < asyncSendThreadNum; i++) {
			...
			NamePreservingRunnable r = new NamePreservingRunnable(new SendWorker(),name);
	...
	private void initCallbackPool() {
		...
		for (int i = 0; i < asyncCallbackThread; i++) {
			...
			NamePreservingRunnable r = new NamePreservingRunnable(new CallbackWorker(),name);
