...
namespace taf
{
    ...
    public:
        ...
        AdapterDescriptor(const std::string &adapterName,const std::string &threadNum,const std::string &endpoint,taf::Int32 maxConnections,const std::string &allowIp,const std::string &servant,taf::Int32 queuecap,taf::Int32 queuetimeout,taf::Int32 grid,const std::string &protocol,const std::string &handlegroup,taf::Int32 shmkey,taf::Int32 shmcap)
            :adapterName(adapterName),threadNum(threadNum),endpoint(endpoint),maxConnections(maxConnections),allowIp(allowIp),servant(servant),queuecap(queuecap),queuetimeout(queuetimeout),grid(grid),protocol(protocol),handlegroup(handlegroup),shmkey(shmkey),shmcap(shmcap)
        {
        }
        AdapterDescriptor()
    ...
    public:
        ...
        static string MD5()
        {
            return "16c160a9ff8d69b893854cebccb6afd0";
        }
        ServerDescriptor(const std::string &application,const std::string &serverName,const std::string &nodeName,const std::string &basePath,const std::string &exePath,const std::string &settingState,const std::string &presentState,const std::string &profile,const std::string &patchVersion,const std::string &patchTime,const std::string &patchUser,const map<std::string, taf::AdapterDescriptor> &adapters,const std::string &serverType,const std::string &startScript,const std::string &stopScript,const std::string &monitorScript,taf::Int32 configCenterPort,taf::Int32 asyncThreadNum,const std::string &setId)
            :application(application),serverName(serverName),nodeName(nodeName),basePath(basePath),exePath(exePath),settingState(settingState),presentState(presentState),profile(profile),patchVersion(patchVersion),patchTime(patchTime),patchUser(patchUser),adapters(adapters),serverType(serverType),startScript(startScript),stopScript(stopScript),monitorScript(monitorScript),configCenterPort(configCenterPort),asyncThreadNum(asyncThreadNum),setId(setId)
        {
        }
        ServerDescriptor()
    ...
    public:
        ...
        ObjectName(const std::string &ObjectId)
            :ObjectId(ObjectId)
        {
        }
        ObjectName()
    ...
    public:
        ...
        ObjectItem(const vector<taf::EndpointF> &vActiveEndpoints,const vector<taf::EndpointF> &vInactiveEndpoints)
            :vActiveEndpoints(vActiveEndpoints),vInactiveEndpoints(vInactiveEndpoints)
        {
        }
        ObjectItem()
        {
