...
void TC_Mysql::execute(const string& sSql)
{
	...
	*/
    ...
    if (iRet != 0)
    {
        throw TC_Mysql_Exception("[TC_Mysql::execute]: mysql_query: [ " + sSql+" ] :" + string(mysql_error(_pstMql)), mysql_errno( _pstMql ) );
    }
}
...
TC_Mysql::MysqlData TC_Mysql::queryRecord(const string& sSql)
{
	...
	*/
    ...
    if(pstRes == NULL)
    {
        throw TC_Mysql_Exception("[TC_Mysql::queryRecord]: mysql_store_result: " + sSql + " : " + string(mysql_error(_pstMql)), mysql_errno( _pstMql ) );
    }
