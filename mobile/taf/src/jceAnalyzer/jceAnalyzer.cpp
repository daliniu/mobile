...
void JceAnalyzer::analyzeWithWupBuffer(const string &file, const string& putName)
{
	...
	else if(_wupVersion == 3)//精简版没有structname,how to do ?
	{
...
int JceAnalyzer::compareTag(int tag, int expectTag, bool bIsRequire)
{
    ...
    // optional tag
	if (expectTag < tag) //JCE 中tag小于从buf读出来的tag
	{
		// 可能对方没有编码expectTag到buf
		//因为是可选，就跳过jce文件中的expectTag
		//并回退AnalyzerDataHead，jce文件中下一个tag会用到
