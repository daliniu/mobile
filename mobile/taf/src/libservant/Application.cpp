...
{
    LOGINFO("Application::cmdViewStatus:" << command << " " << params << endl);
 ...
 bool Application::cmdCloseCoreDump(const string& command, const string& params, string& result)
{
    ...
    if(ret != 0)
    {
        LOG->error()<<"error: "<<strerror(errno)<<endl;
        return false;
    }

    LOG->debug()<<"before :cur:"<<tlimit.rlim_cur<<";max: "<<tlimit.rlim_max<<endl;
    ...
    LOG->debug()<<"after cur:"<<tlimit.rlim_cur<<";max: "<<tlimit.rlim_max<<endl;
...
{
    LOGINFO("Application::cmdSetLogLevel:" << command << " " << params << endl);
...
{
	LOGINFO("Application::cmdEnableDayLog:" << command << " " << params << endl);
	...
	if(!(nNum == 2 || nNum == 3))
	{
		result = "usage: taf.enabledaylog {remote|local}|[logname]|{true|false}";
		return false;
	}
	...
	if(vParams[0] == "local")
	{
		TafTimeLogger::getInstance()->enableLocal(sFile,bEnable);
		return true;
	}

	if(vParams[0] == "remote")
	{
		TafTimeLogger::getInstance()->enableRemote(sFile,bEnable);
		return true;
	}
...
{
    LOGINFO("Application::cmdLoadConfig:" << command << " " << params << endl);
...
{
    LOGINFO("Application::cmdConnections:" << command << " " << params << endl);
...
bool Application::cmdLoadProperty(const string& command, const string& params, string& result)
{
    try
    {
        LOGINFO("Application::cmdLoadProperty:" << command << " " << params << endl);
        ...
        TafTimeLogger::getInstance()->setLogInfo(_communicator, ServerConfig::Log, ServerConfig::Application, ServerConfig::ServerName, ServerConfig::LogPath,setDivision());

        ServerConfig::Config = _conf.get("/taf/application/server<config>");

        TafRemoteConfig::getInstance()->setConfigInfo(_communicator, ServerConfig::Config, ServerConfig::Application, ServerConfig::ServerName, ServerConfig::BasePath,setDivision());
...
{
    LOGINFO("Application::cmdViewAdminCommands:" << command << " " << params << endl);
...
{
    LOGINFO("Application::cmdCloseCout:" << command << " " << params << endl);
...
{
	bool bEnableSet = TC_Common::lower(_conf.get("/taf/application<enableset>", "n"))=="y"?true:false;;

	string sSetDevision = bEnableSet?_conf.get("/taf/application<setdivision>", ""):"";
	return sSetDevision;
}
...
void Application::initializeServer()
{
	...
	//初始化服务是否对空链接进行超时检查
	bool bEnable = (_conf.get("/taf/application/server<emptyconcheck>","0")=="1")?true:false;

	_epollServer->EnAntiEmptyConnAttack(bEnable);
	_epollServer->setEmptyConnTimeout(TC_Common::strto<int>(toDefault(_conf.get("/taf/application/server<emptyconntimeout>"), "3")));
    ...
    //设置日志级别
    string level = _conf.get("/taf/application/server<logLevel>","DEBUG");
    TafRollLogger::getInstance()->logger()->setLogLevel(TC_Common::upper(level));
...
void Application::BusConnectRecover()
{
	map<string, string> m = AppCache::getInstance()->getDomainMap("BusServerConnect");
	for(map<string, string>::iterator it = m.begin(); it != m.end(); ++it)
	{
		TC_EpollServer::BindAdapterPtr pBindAdapter = Application::getEpollServer()->getBindAdapter(it->second);
		if (!pBindAdapter)
		{
			LOG->error() << "[TAF][Application::BusConnectRecover not found adapter handle by " << it->second << "]" << endl;
			return;
		}
cout << "begin connectBusCommu" << endl;
		pBindAdapter->connectBusCommu(it->first);
cout << "end connectBusCommu" << endl;
	}
}
