...
{  
    try
    {
		...
		if (!bCloseCout)
		{
			string sCloseCoutValue = tConf.get("/taf/application/server<closecout>");
...
void parseConfig(int argc, char *argv[])
{
    ...
    if(sConfigFile == "")
    {
        ...
        cerr << argv[0] << " --locator=taf.tafregistry.QueryObj@tcp -h 172.25.38.67 -p 17890"  " --config=config.conf [--nodeid = 172.25.38.67 --registryObj=taf.tafregistry.RegistryObj]" << endl;
    ...
    if(sLocator != "")
    {
        if(sRegistryObj == "")
        {
            sRegistryObj = "taf.tafregistry.RegistryObj";
        }
	bool bCloseOut=true;
