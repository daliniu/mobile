...
int TafRemoteConfig::setConfigInfo(const CommunicatorPtr &comm, const string &obj, const string & app, const string &serverName, const string& basePath,const string& setdivision, int maxBakNum)
{
    _comm           = comm;
    if(!obj.empty())
    {
...
string TafRemoteConfig::getRemoteFile(const string &sFileName, bool bAppConfigOnly)
{
    if (_configPrx)
    {
        string stream;
		int ret;
		if(_sSetdivision.empty())
		{
			ret = _configPrx->loadConfig(_sApp, (bAppConfigOnly ? "" : _sServerName), sFileName, stream);
		}
		else
		{
			struct ConfigInfo confInfo;
			confInfo.appname = _sApp;
			confInfo.servername = (bAppConfigOnly ? "" : _sServerName);
			confInfo.filename = sFileName;
			confInfo.bAppOnly = bAppConfigOnly;
			confInfo.setdivision = _sSetdivision;
