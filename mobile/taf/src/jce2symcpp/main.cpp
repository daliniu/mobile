...
void usage()
{
    cout << "Usage : jce2symcpp [OPTION] jcefile" << endl;
    ...
    cout << "  --with-importc                    如果带有该参数，生成的函数前带有IMPORT_C" << endl;
...
int main(int argc, char* argv[])
{
	...
	if (option.hasParam("with-importc"))
	{
		j2c.setWithImportC(true);
	}
	else
	{
		j2c.setWithImportC(false);
	}
	try
	{
	    for(size_t i = 0; i < vJce.size(); i++)
