...
//指针需要内存时通过偏移指向预分配内存块，减少解码过程中的内存申请
	...
	public:
		...
		/// 设置缓存
		void setMapBuffer(std::vector<char> &buf)
		{
			_buf = &buf[0];
			_buf_len = buf.size();
			_cur = 0;
...
//////////////////////////////////////////////////////////////////
	...
	public:
		...
		void read(tafUInt32& n, uint8_t tag, bool isRequire = true)
		{
			Int64 i = (Int64)n;
			read(i,tag,isRequire);
			n = (tafUInt32)i;
		...
		void read(char *buf, const tafUInt32 bufLen, tafUInt32 & readLen, uint8_t tag, bool isRequire = true)
		{
			if (skipToTag(tag))
			{
				DataHead h;
		...
		void read(std::map<K, V>& m, uint8_t tag, bool isRequire = true)
		{
			if (skipToTag(tag))
			{
				DataHead h;
		...
		void read(std::vector<Char>& v, uint8_t tag, bool isRequire = true)
		{
			if (skipToTag(tag))
			{
				DataHead h;
				...
				case DataHead::eSimpleList:
					{
						...
						v.resize(size);
/*当size为0时，v resize为空  v[0]是空指针 UStl没有vector默认构造, 而且也不应该构造出一个无效实例*/
						if(size > 0)
						{
		...
		template<typename T>
		void read(std::vector<T>& v, uint8_t tag, bool isRequire = true)
		{
			if (skipToTag(tag))
			{
				DataHead h;
		...
		/// 读取结构数组
		template<typename T>
		void read(T* v, const tafUInt32 len, tafUInt32 & readLen, uint8_t tag, bool isRequire = true)
		{
			if (skipToTag(tag))
			{
				DataHead h;
...
//////////////////////////////////////////////////////////////////
	...
	public:
		...
		void write(tafUInt32 n, uint8_t tag)
		{
			write((Int64) n, tag);
		}
		...
		void write(const char *buf, const tafUInt32 len, uint8_t tag)
		{
		...
		template<typename K, typename V>
		void write(const std::map<K, V>& m, uint8_t tag)
		{
		...
		template<typename T>
		void write(const std::vector<T>& v, uint8_t tag)
		{
		...
		template<typename T>
		void write(const T *v, const tafUInt32 len, uint8_t tag)
		{
			DataHead::writeTo(*this, DataHead::eList, tag);
			write(len, 0);
		...
		void write(const std::vector<Char>& v, uint8_t tag)
		{
