...
{
	JceInputStream * stream = [[JceInputStream alloc] init];
...
- (int)readInt1
{
	assert( _headType == JCE_TYPE_INT1 );
...
- (int)readInt2
{
	assert( _headType == JCE_TYPE_INT2 );
...
- (int)readInt4
{
	assert( _headType == JCE_TYPE_INT4 );
...
- (long long)readInt8
{
	assert( _headType == JCE_TYPE_INT8 );
...
- (float)readFloat:(int)tag
{
    [self readHead];
    assert(_headTag == tag);
	assert(_headType == JCE_TYPE_FLOAT);
    float val;
...
- (double)readDouble:(int)tag
{
    [self readHead];
    assert(_headTag == tag);
	assert(_headType == JCE_TYPE_DOUBLE);
    double val;
...
- (NSData *)readData:(int)tag required:(BOOL)required
{
    ...
    {
        [self readHead];
        NSAssert(_headType == JCE_TYPE_SIMPLE_LIST, nil);
        [self readHead];
        NSAssert(_headType == JCE_TYPE_INT1 && _headTag == 0, nil);
...
- (id)readObject:(int)tag required:(BOOL)required description:(Class)theClass
{
    ...
    {
        [self readHead];
        assert(_headType == JCE_TYPE_STRUCT_S);
        ...
        for (int level = 1; level >= 1; )
        {
            ...
            switch (_headType) 
            {
                ...
                case JCE_TYPE_SIMPLE_LIST:
                    [self readHead];
                    assert(_headType == JCE_TYPE_INT1 && _headTag == 0);
        ...
        assert(_headType == JCE_TYPE_STRUCT_E);
        return obj;
    }
}
...
- (NSArray *)readArray:(int)tag required:(BOOL)required description:(id)description
{
    ...
    {
        [self readHead];
        assert(_headType == JCE_TYPE_LIST);
